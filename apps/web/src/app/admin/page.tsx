...
export default function AdminDashboardPage() {
  ...
  return (
    <AdminPage title="Dashboard" description="Live counts from MongoDB. Streaming slots are read from Redis." error={error}>
      {!data ? (
        <p className="mb-4 text-sm text-muted-foreground">Loading metrics...</p>
      ) : (
      ...
      >
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-lg font-semibold">Home slider</h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Choose up to 6 movies for the home page hero slider.
            </p>
          </div>
          <span className="text-sm text-primary underline-offset-4 hover:underline">Open Home slider →</span>
        </div>
      </Link>
