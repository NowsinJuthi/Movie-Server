...
export type AdminProfileSearchHit = {
  id: string;
  name: string;
...
export function AdminProfileSearch({
  ...
  onSelectProfile?: (profile: AdminProfileSearchHit) => void;
  placeholder?: string;
  className?: string;
}) {
  ...
  const suggestQuery = useQuery({
    queryKey: ["admin-profile-suggest", debounced],
    queryFn: () => adminApi.profileSuggest({ q: debounced, limit: 10 }),
    enabled: debounced.length >= 1,
    staleTime: 15_000,
  });

  const hits = suggestQuery.data?.profiles ?? [];
      ...
      {open ? (
        <div className="absolute z-50 mt-1 w-full overflow-hidden rounded-xl border border-border bg-card/98 shadow-[0_18px_50px_-20px_rgb(0_0_0/0.75)] ring-1 ring-primary/15 backdrop-blur-md">
          {suggestQuery.isFetching && hits.length === 0 ? (
            ...
            <p className="px-3 py-2.5 text-sm text-muted-foreground">No profiles found.</p>
          ) : (
